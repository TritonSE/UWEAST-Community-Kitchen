...
const renderNode = (email, secondaryEmails, setSecondaryEmails, updateSecondaryEmails) => {
  ...
  return (
    <div>
      ...
      <FontAwesomeIcon icon={faTrash} className="trash-icon" onClick={(e) => deleteItem()} />
    </div>
  );
};
...
export default function ChangeSecondaryEmailScreen(props) {
  ...
  const { updateSecondaryEmails } = props;
  ...
  const addEmail = (addSecondaryEmail) => {
    ...
    if (
      (addSecondaryEmail && !/^[A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,4}$/i.test(addSecondaryEmail)) ||
      addSecondaryEmail.length === 0
    ) {
  ...
  return (
    <div className="secondary-emails-container">
      ...
      <p className="emailDescription">
        Order confirmations/cancellations will be sent to this email.
      </p>
      ...
      <div className="add-secondary-email">
        ...
        <Button id="submit" className="emailAddButton" onClick={(e) => addEmail(addSecondaryEmail)}>
          {/* The "add" icon */}
