      ...
      <Table aria-label="simple table" className="menuTable">
        <TableHead>
          <TableRow style={{ overflow: "hidden" }}>
            <TableCell className="menuTableHeaders" width="5%">
              Feature
            </TableCell>
            <TableCell className="menuTableHeaders" width="15%" align="center">
              Item Image
            </TableCell>
            <TableCell className="menuTableHeaders" width="10%" align="left">
              Item Name
            </TableCell>
            <TableCell className="menuTableHeaders" width="10%" align="left">
              Category Name
            ...
            <TableCell className="menuTableHeaders" width="30%" align="left">
              Add-ons
            </TableCell>
            <TableCell className="menuTableHeaders" width="10%" align="left">
              Edit
          ...
          {display.displayContent.map((row, index) => {
            ...
            return (
              <TableRow key={row._id} className={bgColor}>
                ...
                <TableCell className="menuRowText" width="10%">
                  {row.itemName}
                </TableCell>
                ...
                <TableCell align="left" className="menuRowText" width="10%">
                  ...
                  <IconButton
                    ...
                    onClick={() => setDeleteConfirmation([row.itemName, row.id])}
                  >
...
async function handleRemoveByID(id, itemList, setItemList, displayContent, setDisplayContent) {
  ...
  await fetch(`${BACKEND_URL}item/remove`, {
    ...
    if (res.ok) {
      ...
      setDisplayContent({
        displayContent: displayContent.displayContent.filter((x) => x.id !== id),
      });
    }
    // invalid token
...
export default function AdminMenuItems(props) {
  ...
  const [display, setDisplay] = useReducer((state, newState) => ({ ...state, ...newState }), {
    searchTerm: "",
    filter: "All",
    displayContent: itemList,
  });
  ...
  const handleDisplayChange = (filter, searchTerm) => {
    ...
    if (searchTerm !== "") {
      workingItems = workingItems.filter((x) =>
        x.itemName.toLowerCase().includes(searchTerm.toLowerCase())
      );
    }
    setDisplay({ filter, searchTerm, displayContent: workingItems });
  };
  /**
  ...
  const handleFeatureChange = async (row) => {
    ...
    const displayContentIndex = display.displayContent.findIndex((x) => x.id === itemID);
      ...
      <div className="adminMenuPageContainer">
        {currentEditItem !== "" && (
          <EditMenuItemModal
            ...
            item={itemList.filter((item) => item.id === currentEditItem)[0]}
            setLoaded={setLoaded}
            setItemEditedSuccess={setItemEditedSuccess}
          />
        )}
        ...
        {addItemModal && (
          <AddMenuItemModal
            ...
            setLoaded={setLoaded}
            setItemAddedSuccess={setItemAddedSuccess}
          />
        )}
        {changeHeaderModal && (
          <ChangeHeaderModal
            changeHeaderModal={changeHeaderModal}
            setChangeHeaderModal={setChangeHeaderModal}
            setLoaded={setLoaded}
        ...
        <div className="aboveTableContainer">
          <div className="addUpdateButtonContainer">
            ...
            <Button
              className="menuChangeHeaderButton"
              onClick={() => {
                setChangeHeaderModal(true);
              }}
            >
              Change Header
            </Button>
