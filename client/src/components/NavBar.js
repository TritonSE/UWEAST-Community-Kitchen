  ...
  return (
    <html>
      ...
      <Navbar className="navbar navbar-bg-color" collapseOnSelect expand="xl" variant="dark">
        ...
        <Navbar.Brand href="/">
          <span>
            <img src={Logo} className="logo-img" alt="UWEAST Logo" width="90" height="90" />
          </span>
        ...
        <div className="cart-icon-container">
          {
            <div className="cart-icon">
              <IconButton onClick={goToMenuPageAndOpenCart}>
                <CartBadge
                  badgeContent={
                    props.itemCount ? props.itemCount : cookies.cart ? cookies.cart.items.length : 0
                  }
                >
        ...
        <Navbar.Collapse id="responsive-navbar-nav">
          <Nav className="ml-auto">
            {/* Menu Page */}
            <Nav.Link className={`nav-link${isPageActive("/")}`} href="/">
              Menu
            </Nav.Link>

            {/* Contact Page */}
            <Nav.Link className={`nav-link${isPageActive("/contact")}`} href="/contact">
              Contact
            </Nav.Link>

            {/* About Page */}
            <Nav.Link className={`nav-link${isPageActive("/about")}`} href="/about">
              About
            </Nav.Link>

            {/* Admin Page */}
            <span className="desktop-tabs">
              <Nav.Link className={adminContentClass + isPageActive("/admin")} href="/admin">
                Admin
              </Nav.Link>
            </span>
            ...
            {/* Logout */}
            <span className="desktop-tabs">
              <Nav.Link className={adminContentClass} onClick={Logout}>
                Logout
              </Nav.Link>
            </span>
            ...
            {/* Login */}
            <span className="desktop-tabs">
              <Nav.Link className={loginButtonClass + isPageActive("/login")} href="/login">
                Login
              </Nav.Link>
            </span>
          </Nav>
