...
const DisplayDateFilters = (filterList, onChange, index, column) => {
  ...
  }/${initialStartDate.getDate()}/${initialStartDate.getFullYear()}`;
  const endDate = `${
    initialEndDate.getMonth() + 1
  }/${initialEndDate.getDate()}/${initialEndDate.getFullYear()}`;
  ...
  return (
    <div style={{ width: "20vw" }}>
      ...
      <DateRangePicker
        initialSettings={{
          startDate: filterList[index][0] || startDate,
          endDate: filterList[index][1] || endDate,
        }}
        onApply={saveDate}
      >
