...
export default class Orders extends React.Component {
  ...
  formatArray(list, date, formatCurrTime, dateSubmission, formatCurrTimeSubmission) {
    const val = formatCurrTime;
    const val2 = formatCurrTimeSubmission;

    const dateOne = `${date}\n${val}`;
    const dateTwo = `${dateSubmission}\n${val2}`;

    const email = this.formatString(list.Customer.Email, 24);
    const name = this.formatString(list.Customer.Name, 14);
    let isCompleted = "Pending Orders";
    if (list.isCompleted === 1) {
      isCompleted = "Completed Orders";
    } else if (list.isCompleted === 2) {
      isCompleted = "Cancelled Orders";
    }

    return [
      list.PayPal.transactionID,
      dateOne,
      name,
      email,
      list.Customer.Phone,
  ...
  getOrders() {
    fetch(`${BACKEND_URL}order`, {
      ...
      .then((data) => {
        ...
        const { length } = getOrdersList;
        const createArr = [];

        for (let i = 0; i < length; i++) {
          // get the date from the database
          const getDate = new Date(getOrdersList[i].Pickup);
          const formatCurrtime = this.formatTime(
            `${getDate.getHours()}:${getDate.getMinutes()}:${getDate.getSeconds()}`
          );
          const date = this.formatDate(getDate);

          // format for the submission date
          const getDateSubmission = new Date(getOrdersList[i].createdAt);
          const formatCurrtimeSubmission = this.formatTime(
            `${getDateSubmission.getHours()}:${getDateSubmission.getMinutes()}:${getDateSubmission.getSeconds()}`
          );
          const dateSubmission = this.formatDate(getDateSubmission);

          const formatedArray = this.formatArray(
            getOrdersList[i],
            date,
            formatCurrtime,
            dateSubmission,
    ...
    return (
      <div className="orders-page-container">
        ...
        <Snackbar
          ...
          onClose={() => this.formatError(false, "")}
        >
