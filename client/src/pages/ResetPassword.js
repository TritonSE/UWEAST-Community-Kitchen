...
export default function ResetPassword() {
  ...
  const handleClickShowOldPassword = () => {
    setState({
      ...state,
      showPassword: { ...state.showPassword, oldPassword: !state.showPassword.oldPassword },
    });
  };
  ...
  const handleClickShowNewPassword = () => {
    setState({
      ...state,
      showPassword: { ...state.showPassword, newPassword: !state.showPassword.newPassword },
    });
  };
  ...
  const handleClickShowNewPasswordConfirmation = () => {
    setState({
      ...
      showPassword: {
        ...state.showPassword,
        confirmNewPassword: !state.showPassword.confirmNewPassword,
      },
    });
  };
  ...
  const handleSubmit = async (event) => {
    ...
    if (state.newPassword.length < 6) {
      ...
      setState({
        ...state,
        errors: { email: false, oldPassword: false, newPassword: true, confirmNewPassword: false },
        form_disabled: false,
        snack: { message: "New Password must be at least 6 characters long.", open: true },
      });
      return;
    }
    ...
    if (state.newPassword !== state.confirmNewPassword) {
      ...
      setState({
        ...state,
        errors: { email: false, oldPassword: false, newPassword: true, confirmNewPassword: true },
        form_disabled: false,
        snack: { message: "New Password Does Not Match", open: true },
      });
      return;
    }
    ...
    if (state.newPassword === state.oldPassword) {
      ...
      setState({
        ...state,
        errors: { email: false, oldPassword: true, newPassword: true, confirmNewPassword: true },
        form_disabled: false,
        snack: { message: "New Password Cannot Be The Same As Current Password!", open: true },
      });
      return;
    }
    ...
    try {
      ...
      // invalid credentials
        ...
        setState({
          ...state,
          errors: { email: true, oldPassword: true, newPassword: false, confirmNewPassword: false },
          form_disabled: false,
          snack: {
            message: "Invalid Authentication: Email or old password not recognized.",
            open: true,
          },
        });
      }
      // any other server response
        ...
        setState({
          ...
          form_disabled: false,
          snack: { message: `An error occurred: Password could not be updated`, open: true },
        });
      }
    } catch (error) {
      // general error
      ...
      setState({
        ...state,
        errors: { email: false, oldPassword: false, newPassword: false, confirmNewPassword: false },
        form_disabled: false,
          ...
          <Grid item md={6} xs={12}>
            <div className="Border">
              ...
              <p className={classes.centered} style={{ color: "#8d8d8d" }}>
                {" "}
                Fill out the fields below to reset the password of an existing account{" "}
              </p>
              {/* form  */}
                  ...
                  InputProps={{
                    endAdornment: (
                      <InputAdornment position="end">
                        ...
                        >
                          {state.showPassword.confirmNewPassword ? (
                            <Visibility />
                          ) : (
                            <VisibilityOff />
                          )}
