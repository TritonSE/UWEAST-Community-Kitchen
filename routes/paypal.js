...
async function inspectValidIPNResponse(req, res) {
  ...
  if (payment_status === "Refunded") {
    ...
    const parent_txt_id = req.body["parent_txn_id"];

    const order = await findOrderByTid(parent_txt_id);
    if (!order) {
      return;
    }
  ...
  const order = await findOrderByTid(txn_id);
  if (!order) {
    return;
  }
...
router.post("/ipn-listener", async (req, res, next) => {
  ...
  const isProduction = config.app.env !== "development";
  // isProduction = false;
...
async function validatePrice(order, payment_amount) {
  ...
  for (let i = 0; i < order.length; i++) {
    ...
    try {
      ...
      const item_object = dict_menuItems[item_id];

      // size price
      let item_price = parseFloat(item_object["Prices"][size]);

      // add to base price by accommodations chosen
      for (let k = 0; k < accommodations.length; k++) {
        const accom_price = parseFloat(
          getAccomPrice(accommodations[k], item_object["Accommodations"])
        );
  ...
  const total = (parseFloat(expected_total) + parseFloat(expected_tax)).toFixed(2);
...
router.post(
  "/validate",
  [body("Order").notEmpty(), body("Amount").notEmpty(), isValidated],
  async (req, res, next) => {
    const valid_order = await validatePrice(req.body.Order, parseFloat(req.body.Amount));
